...
class Transformer(object):
	...
	def __call__(self, inputs, targets=None):
		...
		with tf.variable_scope("Transformer", initializer=initializer):
			...
			encoder_outputs,Awts["Enc"] = self.encode(inputs, attention_bias)

			# Generate output sequence if targets is None, or return logits if target
			# sequence is known.
			if targets is None:
				pred_res , Awts["Dec"] = self.predict(encoder_outputs, attention_bias)
				return  pred_res, Awts
			else:
				logits , Awts["Dec"] = self.decode(targets, encoder_outputs, attention_bias)
				return logits , Awts
	...
	def decode(self, targets, encoder_outputs, attention_bias):
		...
		with tf.name_scope("decode"):
			...
			decoder_self_attention_bias = model_utils.get_decoder_self_attention_bias(
					length)
			outputs , datwt = self.decoder_stack(
	...
	def predict(self, encoder_outputs, encoder_decoder_attention_bias):
		...
		return {"outputs": top_decoded_ids, "scores": top_scores},None
...
class PrePostProcessingWrapper(object):
	...
	def __call__(self, x, *args, **kwargs):
		...
		# Get layer output
		y, wts = self.layer(y, *args, **kwargs)
		...
		for n, layer in enumerate(self.layers):
			...
			with tf.variable_scope("layer_%d" % n):
				with tf.variable_scope("self_attention"):
					encoder_inputs , self.Awts["ESAL_%d"%n]  = self_attention_layer(encoder_inputs, attention_bias)
				with tf.variable_scope("ffn"):
					encoder_inputs = feed_forward_network(encoder_inputs, inputs_padding)

		return self.output_normalization(encoder_inputs), self.Awts
	...
	def call(self, decoder_inputs, encoder_outputs, decoder_self_attention_bias,
		...
		for n, layer in enumerate(self.layers):
			...
			with tf.variable_scope(layer_name):
				with tf.variable_scope("self_attention"):
					decoder_inputs , self.Awts["DSAL_%d"%n] = self_attention_layer(
							decoder_inputs, decoder_self_attention_bias, cache=layer_cache)
				with tf.variable_scope("encdec_attention"):
					decoder_inputs ,self.Awts["DCAL_%d"%n]= enc_dec_attention_layer(
							decoder_inputs, encoder_outputs, attention_bias)
				with tf.variable_scope("ffn"):
					decoder_inputs = feed_forward_network(decoder_inputs)

		return self.output_normalization(decoder_inputs), self.Awts
